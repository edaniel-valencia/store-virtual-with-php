    ...
    public function listarPedidos()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($data[$i]['proceso'] == 1) {
                ...
                $botonAprobar = '<button type="button" onclick="cambiarProceso('.$data[$i]['id'].')" class="btn btn-success"><i class="fas fa-check"></i> Aprobar</button> ';
            } else {
                $data[$i]['estado'] =  '<span class="badge badge-success">COMPLETADO</span>';
                $botonAprobar = '';
            }
            $data[$i]['accion'] = $botonAprobar . '<button type="button" onclick="verDireccion('.$data[$i]['id'].')" class="btn btn-primary">Envio / Comprobante</button>';
        }
