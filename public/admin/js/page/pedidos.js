...
function verDireccion(idPedido) {
  ...
  http.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      const res = JSON.parse(this.responseText);
      const comprobante = res.comprobante
        ? `<li class="list-group-item text-center">
              Comprobante de pago<br>
              <a href="${base_url + 'public/img/comprobantes/' + res.comprobante}" target="_blank">
                <img src="${base_url + 'public/img/comprobantes/' + res.comprobante}" class="img-fluid img-thumbnail mt-2" alt="Comprobante de pago">
              </a>
            </li>`
        : '';
