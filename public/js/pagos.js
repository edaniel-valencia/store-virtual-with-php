...
function configurarComprobante() {
  const frmComprobante = document.querySelector("#frmComprobante");
  frmComprobante.onsubmit = function (e) {
    e.preventDefault();
    if (productos.length == 0) {
      alerta("TU CARRITO ESTA VACIO", 2);
      return;
    }
    const formData = new FormData(this);
    formData.append("productos", JSON.stringify(productos));
    const http = new XMLHttpRequest();
    http.open("POST", ruta + "registro/registrarPedidoManual", true);
    http.send(formData);
